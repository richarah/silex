...
check_empty "grep --vcs: respects .gitignore build/" \
    "$("$SILEX" grep --vcs 'generated' "$T" 2>/dev/null)"
...
check_nonempty "find -S: lowercase pattern matches mixed-case filename" \
    "$("$SILEX" find "$T/ftest" -S -name 'makefile')"

check_empty "find -S: should NOT match with -S and uppercase 'MAKEFILE'" \
    "$("$SILEX" find "$T/ftest" -S -name 'MAKEFILE' 2>/dev/null | grep -i makefile)"
...
check_empty "find --vcs: skips .git/" \
    "$("$SILEX" find "$T/fvcs" --vcs -name 'config' 2>/dev/null)"

# --vcs skips hidden
check_empty "find --vcs: skips hidden files" \
    "$("$SILEX" find "$T/fvcs" --vcs -name '.hidden' 2>/dev/null)"

# --vcs finds regular files
check_nonempty "find --vcs: finds regular source files" \
    "$("$SILEX" find "$T/fvcs" --vcs -name 'main.c' 2>/dev/null)"
...
check_nonempty "find --changed-within 10s: finds just-created file" \
    "$("$SILEX" find "$T" --changed-within 10s -name 'new_file' 2>/dev/null)"

check_empty "find --changed-within 0s: invalid spec rejected" \
    "$("$SILEX" find "$T" --changed-within 0s -name 'new_file' 2>/dev/null)"

check_nonempty "find --changed-within 1m: finds recent file" \
    "$("$SILEX" find "$T" --changed-within 1m -name 'new_file' 2>/dev/null)"

check_nonempty "find --changed-within 1h: finds recent file" \
    "$("$SILEX" find "$T" --changed-within 1h -name 'new_file' 2>/dev/null)"
...
"$SILEX" find "$T" --changed-within 1w -name 'new_file' > /dev/null 2>&1
check "find --changed-within 1w: syntax valid (exit 0)" "0" "$?"

echo "--- SILEX_SMART env var ---"

# SILEX_SMART=1 should enable smart case for grep
check_nonempty "SILEX_SMART=1: grep lowercase matches mixed-case" \
    "$(printf 'Hello World\n' | SILEX_SMART=1 "$SILEX" grep 'hello world')"

check_empty "SILEX_SMART=1: grep uppercase stays sensitive" \
    "$(printf 'Hello World\n' | SILEX_SMART=1 "$SILEX" grep 'HELLO')"

# SILEX_SMART=1 should enable vcs for find
check_nonempty "SILEX_SMART=1: find skips VCS and finds src files" \
    "$(SILEX_SMART=1 "$SILEX" find "$T/fvcs" -name 'main.c' 2>/dev/null)"

check_empty "SILEX_SMART=1: find skips .git/" \
    "$(SILEX_SMART=1 "$SILEX" find "$T/fvcs" -name 'config' 2>/dev/null)"
