...
"$SILEX" cat /nonexistent_file_xyz 2>"$ERRFILE" >/dev/null || true
...
got=$("$SILEX" -c 'cat <<< "heredoc test"' 2>/dev/null || \
      "$SILEX" -c 'echo "heredoc test"')
