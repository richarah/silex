...
"$SILEX" cat "$NULLFILE" > /dev/null 2>&1
check "cat: null-only file doesn't crash" "$?"

"$SILEX" wc -c "$NULLFILE" > /dev/null 2>&1
...
"$SILEX" cat "$LARGEFILE" > /dev/null 2>&1
check "cat: 256KB random binary doesn't crash" "$?"

"$SILEX" wc -c "$LARGEFILE" > /dev/null 2>&1
